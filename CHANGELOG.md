...
release published, site live.** The package version is `4.0.0`; the last tagged
stable release the source installers and the GitHub Action pin to remains
`v3.1.0`. There is no PyPI publication, no safety guarantee, and no vulnerability
confirmation; `manual_review_required` stays true. See
`docs/releases/V4_RELEASE_NOTES.md` and `docs/V4_STABLE_SCOPE.md`.
...
pushed, not published, no GitHub Release, and no post-release cleanup yet. Package
version `3.9.0`, current milestone `v3.9.0`, next milestone `v4.0.0`, latest stable
`v3.1.0` (unchanged, because this is a local-only finalization). A local annotated
`v3.9.0` tag is created and a local backup bundle is written and verified at this
finalization. v3.9.0 adds an internal, additive fixture benchmark harness
(`arkheionx/fixture_harness/`) — a deterministic, local/static regression and
review surface — without changing any existing command behavior or public schema.
There is no public CLI behavior change, no safety guarantee, and no vulnerability
confirmation; `manual_review_required` stays true and `ready_for_submission` stays
false. See `release-notes/v3.9.0.md`.

### Added

- Fixture benchmark runner (`arkheionx/fixture_harness/runner.py`): deterministic,
  local/static per-fixture and per-suite benchmark checks (ID and JSON stability,
  path safety, safety-boundary flags, a no-overclaim scan, and a confined,
  read-only source-text scan). No subprocess, no RPC/fork-url/live-chain/network,
  and no Foundry execution.
- Benchmark snapshot baselines (`arkheionx/fixture_harness/snapshots.py`): a
  committed Set 1 baseline and build/load/compare/drift helpers.
- Real Protocol Fixture Set 2 (AMM/swap, oracle-dependent, upgradeable proxy) and
  Set 3 (bridge/message, liquidation/borrow-repay, governance/timelock) as small
  local/static illustrative source files with pure registries.
- All-fixture benchmark integration: a combined nine-fixture suite,
  ...
  with the Set 1 baseline left unchanged.
- Fixture benchmark crossrefs (`arkheionx/fixture_harness/crossref.py`): a
  standalone, ID-only link to the wider evidence graph with an explicit
  no-overclaim context block; imports and edits no review-package,
  local-validation, or evidence code.
...
- No public CLI command, no new flag, and no changed output key; no file under
  `schemas/` changed; package metadata unchanged at release prep.
- The v3.5 Protocol Intelligence Model, the v3.6 Review Package Workspace, the v3.7
  local-validation behavior, and the v3.8 Protocol Intelligence Core are preserved.

## v3.8.0 - 2026-06-05

Protocol Intelligence Core Machine Leverage. Finalized locally — not pushed, not
published, no GitHub Release, and no post-release cleanup yet. Package version
`3.8.0`, current milestone `v3.8.0`, next milestone `v3.9.0`, latest stable
`v3.1.0` (unchanged, because this is a local-only finalization). The local
annotated `v3.8.0` tag is created and a local backup bundle is written and
verified at this finalization. v3.8.0 adds an internal, additive Protocol
Intelligence Core (`arkheionx/intelligence/`) that connects function roles, value
paths, assumptions, test gaps, a protocol graph, and local-validation coverage
correlation into one deterministic review surface, with optional protocol-graph
context in evidence/report and optional protocol-graph artifacts in the review
package — without changing any existing command behavior or public schema.

### Added

- Function role taxonomy (`arkheionx/intelligence/roles.py`): controlled,
  descriptive review roles by boundary-aware token matching, with deterministic
  role IDs.
- Value path graph model (`arkheionx/intelligence/value_paths.py`): controlled
  value-path kinds with ordered segments, mapped exactly and non-inventively.
- Assumption engine (`arkheionx/intelligence/assumptions.py`): protective
  assumptions per value path with a support level.
- Test gap engine (`arkheionx/intelligence/test_gaps.py`): unproven-today gaps
  with a gap status and a review-priority ordering (not severity).
- Protocol graph builder (`arkheionx/intelligence/graph.py`): a deterministic
  graph of nodes and edges plus a consistency-check layer.
- Local validation coverage correlation (`arkheionx/intelligence/coverage.py`):
  explicit local-validation/trace-receipt IDs mapped onto graph test gaps as
  supporting context.
- Evidence/report protocol graph context: an optional, additive
  `protocol_graph_context` block in evidence packages and report drafts.
- Review Package protocol graph artifact inclusion: optional discovery,
  classification, validation, cross-reference, and deterministic export of
  `.arkheionx/out/protocol-graph/` artifacts (seven optional manifest kinds).
- Protocol Intelligence Core documentation (`docs/PROTOCOL_INTELLIGENCE_CORE.md`),
  a graph workflow guide (`docs/PROTOCOL_GRAPH_WORKFLOW.md`), a graph smoke-test
  guide (`docs/PROTOCOL_GRAPH_SMOKE_TEST.md`), plus review-package,
  local-validation, CLI-reference, public-surface, README, and roadmap alignment.

### Safety and Compatibility

- Exact-only linking (exact ID or explicit alias; no fuzzy or substring matching,
  no invented links) and deterministic IDs (SHA-256 over canonical seeds; no
  timestamps, randomness, or process `hash()`).
- No new graph CLI and no protocol-graph writer; no new public command, no new
  CLI flag, and no changed CLI output (`review-package` only gains optional
  graph-artifact inclusion, so its `artifact_count` may rise).
- No RPC, no fork-url, no private keys, no seed phrases, no live-chain calls, no
  transaction broadcasting, no exploit automation, and no auto-submit.
- Manual review required and `ready_for_submission` false on every output; no
  automatic `HUMAN_REVIEWED`.
- No confirmed-vulnerability, final-severity, audit-passed, or bounty-eligibility
  claims; graph consistency does not prove safety, a graph warning does not prove
  a vulnerability, a passing local test does not prove safety, and missing
  coverage does not prove a vulnerability.
- Additive only: no existing artifact shape or public schema changed, and no file
  under `schemas/` changed. The v3.4 evidence/report/evidence-links, v3.5
  Protocol Intelligence Model, v3.6 Review Package Workspace, and v3.7
  local-validation contracts are preserved; old behavior works when graph
  artifacts/context are absent.
- QA / release-candidate gate passed (Agent 11, `READY_FOR_V3_8_RELEASE_PREP`):
  protocol intelligence core, graph context, and review-package graph smokes,
  JSON purity, 519 intelligence tests, 135 evidence/report tests, 155
  review-package tests, 218 local-validation/public tests, full suite 1562 OK
  (skipped=1), repository checks, and writable `make validate` all green.
- Metadata is finalized to `3.8.0` (current `v3.8.0`, next `v3.9.0`, stable
  `v3.1.0` unchanged), and the local annotated `v3.8.0` tag and backup bundle are
  created at this local finalization; not pushed, not published, no GitHub
  Release, and no post-release cleanup. Any push or publication requires explicit
  approval.

## v3.7.0 - 2026-06-04

Foundry / Local Validation Integration and Execution Evidence Layer. Finalized
locally — not pushed, not published, no GitHub Release, and no post-release
cleanup yet. Package version `3.7.0`, current milestone `v3.7.0`, next milestone
`v3.8.0`, latest stable `v3.1.0` (unchanged, because this is a local-only
finalization). The local annotated `v3.7.0` tag is created and a local backup
bundle is written and verified at this finalization. v3.7.0 adds a local-first
way to ingest a saved Foundry test output file into deterministic
local-validation artifacts that connect to the existing Arkheionx evidence graph,
without changing any existing command behavior or public schema.

### Added

- Local Validation model and deterministic IDs
  (`arkheionx/local_validation/model.py`, `ids.py`): run, test-result,
  trace-receipt, artifact, and summary dataclasses with SHA-256 IDs over
  canonical seeds (no timestamps, randomness, or process `hash()`).
- Foundry saved-output parser (`arkheionx/local_validation/parser.py`):
  structured-first with a conservative text fallback, plus committed local-only
  Foundry output fixtures under `tests/fixtures/local_validation/foundry/`.
- Local Validation builder (`arkheionx/local_validation/builder.py`): a pure
  ...
  Model by exact match only (unresolved and ambiguous links are warnings).
- Local Validation writer (`arkheionx/local_validation/writer.py`): deterministic,
  path-safe, secret-safe `.arkheionx/out/local-validation/` artifacts with an
  artifact index and `checksums/SHA256SUMS`.
- `arkheionx local-validate <repo> --input PATH` CLI command (additive), with
  `--json`, `--no-write`, `--output`, `--format`, `--tool`, and provenance-only
  `--command`.
- Review Package local-validation inclusion: optional collection, the six
  optional manifest kinds, additive validation and cross-reference checks, and
  deterministic export inclusion.
- Evidence/report support context: optional `local_validation_support` block and
  a "Local Validation Context" report section, supporting context only.
- Local Validation documentation (`docs/LOCAL_VALIDATION.md`), a workflow guide
  (`docs/LOCAL_VALIDATION_WORKFLOW.md`), and a local smoke-test guide
  (`docs/LOCAL_VALIDATION_SMOKE_TEST.md`), plus CLI-reference, public-surface,
  review-package, README, and roadmap alignment.

### Safety and Compatibility

- Saved Foundry output only: no live forge runner, no subprocess runner, and no
  Foundry runtime requirement for ingestion.
- No RPC, no fork-url, no private keys, no seed phrases, no live-chain calls, no
  transaction broadcasting, no exploit automation, and no auto-submit; the
  provenance-only `--command` is never executed.
- Manual review required and `ready_for_submission` false on every output; no
  automatic `HUMAN_REVIEWED`.
- No confirmed-vulnerability, final-severity, audit-passed, or bounty-eligibility
  claims; a passing local test is supporting context only and a failing local
  test needs manual review.
- The trace-bounded `EVIDENCE_READY` rule is unchanged: a tested-only local
  signal does not create it.
- Additive only: `local-validate` is a new command; no existing command name,
  flag, behavior, artifact shape, or public schema changed, and no file under
  `schemas/` changed. v3.4 evidence/report/evidence-links, the v3.5 Protocol
  Intelligence Model, and the v3.6 Review Package Workspace contracts are
  preserved.
- QA / release-candidate gate passed (Agent 10, `READY_FOR_V3_7_RELEASE_PREP`):
  local-validate, review-package, and evidence/report smokes, JSON purity, 205
  local-validation tests, 104 review-package tests, 82 evidence/report tests, 66
  intelligence/public tests, full suite 1009, repository checks, and writable
  `make validate` all green.
- Metadata is finalized to `3.7.0` (current `v3.7.0`, next `v3.8.0`, stable
  `v3.1.0` unchanged), and the local annotated `v3.7.0` tag and backup bundle are
  created at this local finalization; not pushed, not published, no GitHub
  Release, and no post-release cleanup. Any push or publication requires explicit
  approval.
...
Review Package Workspace, Strict Export, and Validation Bundle. Finalized
locally — not pushed, not published, no GitHub Release, and no post-release
cleanup yet. Package version `3.6.0`, current milestone `v3.6.0`, next milestone
`v3.7.0`, latest stable `v3.1.0` (unchanged, because this is a local-only
finalization). v3.6.0 adds a local-first review package layer that assembles
existing Arkheionx review artifacts into one structured, checksummed, validated,
and optionally exported package for human review, without changing any existing
command behavior or public schema.

### Added

- Review Package Workspace: a local, reviewer-ready package assembled from
  existing `.arkheionx/out/` artifacts.
- Internal review package model (`arkheionx/review_package/model.py`): manifest,
  artifact, validation-result, and export dataclasses.
- Deterministic review package IDs (`ids.py`): `review-package:` package,
  validation, export, and per-artifact IDs from canonical seeds (no timestamps,
  randomness, or process `hash()`).
- Artifact collector and manifest builder (`collector.py`, `manifest.py`):
  discovery, repo-relative paths, required/optional sets, and the safety
  boundary block.
- Checksum and validation layer (`checksums.py`, `validate.py`): sha256
  `SHA256SUMS`, plus shape, required-artifact, path-safety, checksum,
  secret-pattern (class-only), safety-boundary, and cross-reference checks.
- `arkheionx review-package` CLI command (additive), with `--no-write` dry run,
  default write, and `--strict` (exit `1` when validation is invalid or reports
  errors).
- Local package folder: `manifest.json`, `validation.json`, `README.md`,
  `limitations.md`, copied `artifacts/`, and `checksums/SHA256SUMS`.
- Deterministic local ZIP export (`export.py`): stable archive root, sorted
  relative-only entries, fixed entry metadata, exports/self/cache exclusions.
- Protocol-model sidecar (`intelligence.py`):
  `artifacts/intelligence/protocol-model.json` when buildable, repo path
  scrubbed.
- Package-level cross-reference validation (`crossref.py`): exact ID match or
  explicit alias only — no fuzzy or substring matching; unresolved references
  are warnings, never invented links.
- Reviewer docs and a local smoke workflow (`docs/REVIEW_PACKAGE.md`,
  `docs/REVIEW_PACKAGE_WORKFLOW.md`, `docs/REVIEW_PACKAGE_SMOKE_TEST.md`).

### Compatibility and Safety

- Additive only: `review-package` is a new command; no existing command name,
  flag, behavior, artifact shape, or public schema changed.
- v3.4 contracts preserved (trace-bounded evidence readiness, evidence package
  manifests, report draft / manual-review behavior, `evidence-links` read-only
  behavior); the v3.5 Protocol Intelligence Model is preserved and reused.
- Safety boundaries unchanged: local/static only, no RPC, no private keys, no
  seed phrases, no live-chain, no broadcast, no exploit automation, no
  auto-submit, no automatic `HUMAN_REVIEWED`, no confirmed vulnerabilities, no
  final severity, no audit-passed claim, no bounty outcome claim, manual review
  required, `ready_for_submission` false.
- QA / release-candidate gate passed (Agent 9, `READY_FOR_V3_6_RELEASE_PREP`):
  152 review-package tests, 90 intelligence tests, 103 v3.4 contract tests, full
  suite 753, repository checks, and writable `make validate` all green.
- Metadata is finalized to `3.6.0` (current `v3.6.0`, next `v3.7.0`, stable
  `v3.1.0` unchanged) and the local annotated `v3.6.0` tag is created at this
  local finalization; not pushed, not published, and no GitHub Release.

## v3.5.0 - 2026-06-03

Protocol Intelligence Model. Finalized locally — not pushed, not published, no
GitHub Release, and no post-release cleanup yet. Package version `3.5.0`, current
milestone `v3.5.0`, next milestone `v3.6.0`, latest stable `v3.1.0` (unchanged,
because this is a local-only finalization). v3.5.0 adds an internal, additive
Protocol Intelligence Model that connects contracts, functions, value paths,
assumptions, test gaps, proof suggestions, proof/trace receipts, evidence
packages, report drafts, and evidence-links through shared deterministic IDs,
without changing any public command, artifact, or schema.

### Added

- Internal Protocol Intelligence Model package (`arkheionx/intelligence/`):
  internal-only, no new CLI command, no public schema change.
- Stable deterministic IDs (`ids.py`): `protocol_id`, `contract_id`,
  `function_id`, `value_path_id`, `assumption_id`, `test_gap_id`,
  `proof_suggestion_id`, `proof_receipt_id`, `trace_receipt_id`,
  `evidence_package_id`, `report_id`, `evidence_link_id` — SHA-256 over a
  canonical seed, with no timestamps, randomness, or process `hash()`.
- `ProtocolModel` dataclasses (`model.py`) with per-entity nodes, each carrying
  an `aliases` map (legacy IDs/slugs/display names) and a `metadata` map.
- `ProtocolModel` builder (`build.py`) from `analyze()` and review-map outputs,
  with alias-based function resolution.
- Additive review-map intelligence IDs (`contract_id` / `function_id`) that keep
  every existing review-map key and artifact present.
- Reasoning links: value paths -> assumptions -> test gaps -> proof suggestions.
- Artifact links: proof -> trace -> evidence package -> report draft ->
  evidence-links.

### Compatibility and Safety

- Old v3.4 artifacts without intelligence fields remain valid; v3.5 fields are
  additive and old IDs are preserved as aliases.
- No mandatory `protocol-model.json` sidecar; no public schema-breaking
  migration; no CLI redesign.
- v3.4 contracts preserved: trace-bounded evidence readiness, evidence package
  manifests, report draft/manual-review behavior, and `evidence-links`
  read-only behavior are unchanged.
- Safety boundaries unchanged: local/static only, no RPC, no private keys, no
  live-chain, no broadcast, no exploit automation, no auto-submit, no automatic
  `HUMAN_REVIEWED`, no confirmed vulnerabilities, no final severity, no bounty
  eligibility, manual review required.
- Agent 8 QA gate passed (`READY_FOR_V3_5_RELEASE_PREP`): 90 targeted
  intelligence tests, 103 v3.4 contract tests, full suite 601, repository checks,
  and writable `make validate` all green. The final local release re-ran
  validation, set `v3.5.0` metadata, and created the local `v3.5.0` tag; no push
  or publication.

## v3.4.0 - 2026-06-03

Proof and Evidence Workflow Deepening. This release finalizes v3.4.0 locally
and connects the proof, trace, evidence package, report draft, evidence-links,
and review-map workflow. No push, publication, GitHub Release, or post-release
cleanup has been performed.

### Added

- Trace-bounded evidence readiness: execution proof plus a real linked trace can
  reach `EVIDENCE_READY`; missing trace remains `EXECUTION_CONFIRMED`.
- Deterministic evidence package IDs and additive evidence manifests with proof
  and trace source records.
- Report draft evidence linkage through `evidence_context`,
  `receipt_references`, `report_readiness`, and bounded `claim_references`.
- Enriched `evidence-links` and `review-map` backfill from local proof, trace,
  evidence, and report receipts.

### Compatibility and Safety

- `evidence --from-proof` does not fake missing trace sources.
- `prove --json --no-artifacts` emits pure JSON and writes no proof artifacts.
- `validate-artifacts` catches evidence-ready packages without linked traces,
  stale manifest existence flags, source path mismatches, and manifest linkage
  contradictions.
- Reports remain draft/manual-review only: no automatic `HUMAN_REVIEWED`, no
  final severity, no vulnerability confirmation, and no submission readiness.
- Final Agent 8 validation passed with 511 tests, 2 skipped, writable
  `make validate`, and end-to-end smoke.

## v3.3.0 - 2026-06-02

Focused Review Commands. This release promotes key review-map artifacts into
direct local/static focused commands.

### Added

- Focused review-map commands:
  - `arkheionx test-gap-map`
  - `arkheionx value-paths`
  - `arkheionx assumptions`
  - `arkheionx proof-plan`
  - `arkheionx evidence-links`
- Cross-command integration coverage for shared options, JSON purity,
  `--no-write`, target filtering, existing-artifact reads, and execution
  artifact boundaries.

### Compatibility and Safety

- Preserved artifact-compatible JSON payloads for `test-gap-map.json`,
  `value-paths.json`, `assumptions.json`, `proof-plan.json`, and
  `evidence-links.json`.
- Preserved local/static safety boundaries.
- Preserved `review-map --json` compatibility and existing review-map artifact
  filenames.
- `proof-plan` is planning-only and does not execute proofs.
- `evidence-links` is read-only and does not create evidence or promote
  evidence levels.

## v3.2.0 - 2026-06-02

Developer-Native Review Map and Local Artifact Foundation. Package version is
`3.2.0`; `STABLE_RELEASE` remains `v3.1.0` for the final release commit and
moves to `v3.2.0` in post-release cleanup. Current milestone `v3.2.0`, next
`v3.3.0`. This release line makes Arkheionx easier to use as a local-first
pre-audit readiness workflow without changing the safety boundary: no RPC by
default, no private keys or seed phrases, no transaction broadcasting, no
exploit automation, no auto-submit, no vulnerability confirmation, no final
severity for heuristic signals, and human review required.

### Added

- `arkheionx/cli_ui.py`: an isolated, standard-library `TerminalUI` (banner,
  phases with an honest spinner, sections, results, summary) used by the human
  `review-map` output. Animation and color are gated; artifacts and JSON are
  never colored.
- `docs/START_HERE.md` (60-second onboarding) and `docs/CLI_UX.md` (terminal
  behaviour and the `ARKHEIONX_COLOR` / `ARKHEIONX_NO_ANIMATION` / `NO_COLOR` /
  `ARKHEIONX_ASCII` / `CI` controls), linked from the README.
- `tests/test_cli_ui.py` and new `review-map` CLI UX tests covering animation
  and color gating, machine-readable purity, no-write claims, summary-count
  accuracy, and spinner-thread cleanup.
- Test Gap Map artifacts for `review-map`: `test-gap-map.json` and
  `test-gap-map.md`, written alongside the existing artifacts. The Test Gap Map
  organizes existing test gaps by target, review priority, confidence, scenario
  category, related value paths/assumptions, proof suggestion, and evidence
  status to answer "which tests or local proofs should I write first?". It is a
  derived view of the in-memory review map (reusing the canonical
  `gap-<contract>-<function>` ids), with a new permissive
  `schemas/test-gap-map.schema.json`. No new command or flag; `review-map --json`
  output and the existing artifacts are unchanged. Priority is review order, not
  severity; output stays review guidance only (local/static, no RPC, no keys).
- v4 roadmap, product-positioning, and Sprint 0 architecture documentation that
  frames the Protocol Security Control Plane as product direction, not as a
  completed v3.2.0 runtime surface.

### Changed

- `arkheionx review-map` human output now uses `TerminalUI` for progressive,
  developer-native presentation. JSON and artifact output are byte-for-byte
  unchanged.
- `arkheionx open` human output now uses the same `TerminalUI` DX layer (banner,
  local/static mode line, one honest premeasured analysis step, a counts
  Summary, Top Surfaces, Money Flow, Next, and a Boundary reminder). `--json`
  output, exit codes, and the no-artifacts behavior are unchanged.
- Public docs clarify the current-vs-planned boundary: v3.2.0 is the
  review-map/local-artifact foundation, while v4 workspace features such as
  baselines, diffs, sessions, reviewer notes, policy profiles, and review
  packages remain planned.

### Compatibility and Deferred Governance

- `review-map --json` remains pure JSON, and `review-map --no-write` still
  writes no artifacts.
- `.arkheionx/out/` outputs remain generated, local, gitignored, and not
  intended to be committed in v3.2.0. No `.arkheionx/` gitignore carve-out is
  introduced.
- `EVIDENCE_READY` is the canonical evidence package term. `REPORT_READY` is
  treated as legacy protocol-era terminology; no schema, validator, or enum
  migration is introduced before v3.2.0.
- `HUMAN_REVIEWED` remains manual-attestation only and is not emitted
  automatically.
- `validate-artifacts` keeps the current 0/1/2 behavior: `0` clean validation,
  `1` validation issues or review attention, `2` command/input failure. A
  dedicated artifact-validation failure code is deferred.
- Schema dialect unification and schema registry work are deferred to a later
  governance release.
...
unchanged. Package version is `3.0.0`; `STABLE_RELEASE` remains `v2.10.0` until
v3.0.0 is published, then moves to `v3.0.0` in post-release cleanup.
...
- Version metadata to `3.0.0` (stable `v2.10.0`, current `v3.0.0`, next
  `v3.1.0`).
...
- Public-surface, stability-contract, v3-readiness, roadmap, release-checklist,
  and GitHub-repo-surface docs aligned to the v3.0.0 public stable launch.
...
- v3 launch tests: version `3.0.0`, README v3 positioning and

"""Morpho Midnight protocol lens (``morpho-midnight``).

Models Morpho Midnight as a protocol-specific economic system rather than generic
DeFi: a fixed-maturity credit market with credit/debt unit accounting,
...
LENS_ID = "morpho-midnight"

_BOUNDARY = (
    "The Morpho Midnight lens models a protocol; it does not confirm vulnerabilities, "
...
# --------------------------------------------------------------------------
# Behavior promises (section 8: PROMISE-MM-01 .. PROMISE-MM-14)
# --------------------------------------------------------------------------
def _promises() -> list[m.BehaviorPromise]:
    P = m.BehaviorPromise
    return [
        P("PROMISE-MM-01", "Fixed maturity: debt should not be increasable after maturity, while unwind paths remain possible.",
        ...
        P("PROMISE-MM-02", "Credit/debt unit conservation: credit and debt units must move consistently with actual asset settlement.",
        ...
        P("PROMISE-MM-03", "Settlement-time liquidity: offers do not lock maker capital, but settlement must not finalize economic state without required payment.",
        ...
        P("PROMISE-MM-04", "Maker group exposure cap: a maker's group budget must cap exposure across offers and markets according to signed intent.",
        ...
        P("PROMISE-MM-05", "Multi-collateral solvency: borrowers must not withdraw collateral or end a transaction unsafe beyond intended liquidation rules.",
        ...
        P("PROMISE-MM-06", "Bad-debt socialization: bad debt must reduce lender credit consistently once realized.",
        ...
        P("PROMISE-MM-07", "Liquidation consistency: different liquidation input modes must not create materially different economic outcomes unless documented and harmless.",
        ...
        P("PROMISE-MM-08", "Post-maturity debt handling: after maturity, outstanding debt should remain unwindable or liquidatable according to protocol rules.",
        ...
        P("PROMISE-MM-09", "Gates: gates should restrict entry without trapping safe exits or breaking accounting.",
        ...
        P("PROMISE-MM-10", "Fees: settlement fees, continuous fees, referral fees, and claimable fees must not create unbacked value.",
        ...
        P("PROMISE-MM-11", "Periphery target correctness: target-based periphery functions must not treat target completion using the wrong unit, wrong side, or wrong net/gross amount.",
        ...
        P("PROMISE-MM-12", "Cap correctness: maxAssets, maxUnits, targetAssets, and targetUnits must cap the economically relevant amount.",
        ...
        P("PROMISE-MM-13", "Callback finality: callbacks may observe intermediate state only if final transaction invariants still prevent economic violation.",
        ...
        P("PROMISE-MM-14", "Market identity isolation: state, signatures, ratifications, offers, groups, and accounting must not be reusable across markets unless explicitly intended.",
...
# --------------------------------------------------------------------------
# Economic invariants (section 9: INV-MM-01 .. INV-MM-12)
# --------------------------------------------------------------------------
def _invariants() -> list[m.EconomicInvariant]:
    I = m.EconomicInvariant
    return [
        I("INV-MM-01", "Any credit increase, debt reduction, collateral withdrawal, or receiver payout must be backed by actual payment or valid existing position reduction.",
        ...
        I("INV-MM-02", "Debt reduced must not exceed actual loan assets paid, adjusted only by documented mechanics.",
        ...
        I("INV-MM-03", "Collateral withdrawal must only be possible when final position health is valid under final debt, fees, and lossFactor.",
        ...
        I("INV-MM-04", "Credit minted or preserved must correspond to future withdrawable backing after fees, bad debt, and lossFactor.",
        ...
        I("INV-MM-05", "withdrawable must not exceed loan token value actually owed to lenders after repayments, liquidations, fees, and bad debt.",
        ...
        I("INV-MM-06", "For any maker and group, total economic exposure must not exceed the intended cap.",
          relevant_functions=["take", "fill", "settle"],
          state_variables=["group", "consumed", "maxAssets", "maxUnits"],
        ...
        I("INV-MM-07", "A target-based periphery function must not stop early unless the actual user-intended target has been achieved.",
          relevant_functions=["bundle", "settle", "take", "fill"],
        ...
        I("INV-MM-08", "If a path includes fees or receiver payouts, caps and targets must apply to the correct gross or net amount.",
        ...
        I("INV-MM-09", "One actor must not force disproportionate loss onto another actor through undocumented delay or stale state.",
        ...
        I("INV-MM-10", "Accounting or authorization from one market must not affect another market unless explicitly designed.",
        ...
        I("INV-MM-11", "Gate logic must not create inconsistent states where entry is blocked but safe exit, repayment, or liquidation becomes broken.",
        ...
        I("INV-MM-12", "Candidates near known bug families must be tested against a patch model before being considered new.",
...
def _lanes() -> list[m.LensLaneDef]:
    ...
    return [
        L("LANE-01", "Periphery target accounting", "periphery-target-accounting",
          ...
          ("INV-MM-07", "INV-MM-08", "INV-MM-12"), ("PROMISE-MM-11", "PROMISE-MM-12"),
        ...
        L("LANE-02", "Collateral movement after composed target", "collateral-after-composed-target",
          ...
          ("INV-MM-03", "INV-MM-07"), ("PROMISE-MM-05", "PROMISE-MM-11"),
        ...
        L("LANE-03", "Settlement-time liquidity", "settlement-time-liquidity",
          ...
          ("INV-MM-01", "INV-MM-02"), ("PROMISE-MM-03", "PROMISE-MM-13"),
        ...
        L("LANE-05", "Bad debt and lossFactor", "bad-debt-loss-factor",
          ...
          ("INV-MM-04", "INV-MM-05", "INV-MM-09"), ("PROMISE-MM-06",),
        ...
        L("LANE-06", "Liquidation consistency", "liquidation-consistency",
          ...
          ("INV-MM-02", "INV-MM-03"), ("PROMISE-MM-07", "PROMISE-MM-08"),
        ...
        L("LANE-09", "Fee correctness", "fee-correctness",
          ...
          ("INV-MM-01", "INV-MM-08"), ("PROMISE-MM-10",),
...
def _value_flow_templates() -> list[m.ValueFlowPath]:
    ...
    return [
        ...
        V("VF-MM-05", "Liquidation", entry="liquidator targets an unhealthy position",
          movement="debt repaid, collateral seized, bad debt realized", exit="seized collateral payout",
          signals=["liquidate", "lif", "lossFactor"]),
    ]


class MorphoMidnightLens(ProtocolLens):
    """Protocol lens for Morpho Midnight (``morpho-midnight``)."""

    def meta(self) -> m.ProtocolLensMeta:
        return m.ProtocolLensMeta(
            lens_id=LENS_ID,
            display_name="Morpho Midnight Protocol Lens",

...
def render_evidence(pkg: EvidencePackage, project: str) -> str:
    ...
    package_id = str(payload.get("evidence_package_id", ""))
    trace_status = str((payload.get("trace_summary", {}) or {}).get("status", "linked" if pkg.trace_found else "missing"))
    out = [
        "ARKHEIONX EVIDENCE",
        f"Project: {project}",
        f"Target: {pkg.target}",
    ...
    if isinstance(lvs, dict):
        insert = [
            ...
            f"  Tested: {lvs.get('tested_count', 0)}  Trace-bound: {lvs.get('trace_bound_count', 0)}  Needs review: {lvs.get('needs_review_count', 0)}",
    ...
    if isinstance(pgc, dict):
        insert = [
            ...
            f"  Coverage context: {pgc.get('coverage_count', 0)} (tested {pgc.get('tested_count', 0)}, "
            f"trace-bound {pgc.get('trace_bound_count', 0)})",
            "  Supporting context only; manual review required; not ready for submission.",
        ]
        limits_at = out.index("Limits") - 1
        out[limits_at:limits_at] = insert
...
def evidence_text(payload: dict) -> str:
    ...
    if isinstance(lvs, dict):
        lines += [
            "",
            "Local Validation Support (supporting context only):",
            f"- tested support: {lvs.get('tested_count', 0)}",
            f"- trace-bound support: {lvs.get('trace_bound_count', 0)}",
            f"- local tests needing manual review: {lvs.get('needs_review_count', 0)}",
    ...
    if isinstance(pgc, dict):
        lines += [
            "",
            "Protocol Graph Context (supporting context only):",
            f"- function-role context observed: {pgc.get('function_role_count', 0)}",
            f"- value-path context observed: {pgc.get('value_path_count', 0)}",
            f"- assumption context observed: {pgc.get('assumption_count', 0)}",
            f"- test-gap context observed: {pgc.get('test_gap_count', 0)}",
            f"- local-validation coverage context observed: {pgc.get('coverage_count', 0)} "
            f"(tested {pgc.get('tested_count', 0)}, trace-bound {pgc.get('trace_bound_count', 0)})",
            f"- graph review checks: {pgc.get('warning_count', 0)} warnings, {pgc.get('error_count', 0)} errors (review context only)",
            f"- manual review required: {pgc.get('manual_review_required', True)}",
            f"- ready for submission: {pgc.get('ready_for_submission', False)}",
        ]
    return "\n".join(lines) + "\n"

...
def check() -> list[str]:
    ...
    if "## v1.8.0" not in changelog or "## v1.8.0 - Unreleased" in changelog:
        failures.append("CHANGELOG.md does not treat v1.8.0 as released")
    if CURRENT_MILESTONE not in roadmap or "Pre-v2 CLI Candidate current milestone" not in roadmap:
        failures.append(f"docs/ROADMAP.md does not mark {CURRENT_MILESTONE} as current CLI milestone")
    if NEXT_MILESTONE not in roadmap or "Installable Arkheionx CLI / Package" not in roadmap:
    ...
    stale = re.findall(r"Latest stable release: \*\*(?:v0\.[^*]+|v1\.0\.1[^*]*|v1\.1\.[^*]*|v1\.2\.0[^*]*|v1\.3\.0[^*]*|v1\.4\.0[^*]*|v1\.5\.0[^*]*|v1\.6\.0[^*]*|v1\.7\.0[^*]*)", readme)
    if stale:

...
GEN_BEGIN = "<!-- BEGIN GENERATED VERIFICATION SUMMARY -->"
GEN_END = "<!-- END GENERATED VERIFICATION SUMMARY -->"
MANUAL_BEGIN_RE = re.compile(r"<!--\s*BEGIN MANUAL:[^>]*-->", re.IGNORECASE)
MANUAL_END_RE = re.compile(r"<!--\s*END MANUAL:[^>]*-->", re.IGNORECASE)
...
def render_report(e: dict) -> str:
    ...
    lines.append("Generated by `scripts/generate_verification_report.py` from "
                 "`metadata/registry.json`. Content between the BEGIN/END "
                 "GENERATED VERIFICATION SUMMARY markers is regenerated on "
                 "every run. Manual sections (run transcripts, smoke-test "
                 "evidence, archival verification logs) must live OUTSIDE "
                 "those markers and will be preserved.")
    lines.append("")
...
def _extract_manual_blocks(text: str) -> list[str]:
    """Return all `<!-- BEGIN MANUAL: ... --> ... <!-- END MANUAL: ... -->`
    blocks in `text`, in order. Used to rescue manual content from legacy
    reports that were edited before the GENERATED markers existed.
    """
    blocks: list[str] = []
    pos = 0
    while True:
        m = MANUAL_BEGIN_RE.search(text, pos)
        if not m:
            break
        end = MANUAL_END_RE.search(text, m.end())
        if not end:
            break
        blocks.append(text[m.start():end.end()])
        pos = end.end()
    return blocks


def merge_with_manual(generated: str, existing: str) -> str:
    """Replace the generated block in `existing` with `generated`, preserving
    any manual content that lives OUTSIDE the markers.

    Three cases:
    1. Existing has GEN markers: replace just that block, keep prefix/suffix.
    2. Existing has no GEN markers but has BEGIN MANUAL / END MANUAL blocks
       (legacy report with manual edits): emit `generated` followed by the
       extracted manual blocks.
    3. Existing has neither: return `generated`.
    """
    if not existing:
        return generated
    ...
    if b != -1 and e != -1 and e > b:
        e_end = e + len(GEN_END)
        return existing[:b] + generated + existing[e_end:]
    manual = _extract_manual_blocks(existing)
    if manual:
        return generated + "\n\n" + "\n\n".join(manual) + "\n"
    return generated
...
def main() -> int:
    ...
    for e in entries:
        ...
        merged = merge_with_manual(report, current)
        if current == merged:
            continue
        if args.check:

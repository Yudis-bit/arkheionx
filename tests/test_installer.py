...
def installer_env(tmp: Path) -> dict[str, str]:
    env = os.environ.copy()
    env.update(
...
def is_build_dependency_resolution_failure(output: str) -> bool:
    return "setuptools" in output and any(
        phrase in output
        for phrase in [
...
class InstallerScriptTests(unittest.TestCase):
    ...
    def test_default_repo_and_ref_documented(self) -> None:
        ...
        self.assertIn("v8.0.1", text)  # stable tag default (ARKHEIONX_STABLE_TAG)
    ...
    def test_install_writes_receipt_and_uninstall_removes_it(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if install.returncode != 0 and is_build_dependency_resolution_failure(install_output):
                self.skipTest(
                    "install.sh venv install needs release-capable pip build-dependency "
                    "resolution for setuptools>=61"
                )
...
class OnboardingSurfaceTests(unittest.TestCase):
    ...
    def test_version_metadata(self) -> None:
        ...
        self.assertEqual(__version__, "9.1.0.dev0")
        self.assertEqual(STABLE_RELEASE, "v8.0.1")
        self.assertEqual(CURRENT_MILESTONE, "v9.1.0-dev")
        self.assertEqual(NEXT_MILESTONE, "v9.1.0")
